...
export type ApiCallFormat = "openai" | "gemini" | "local";
...
function isAiConfigReady(config: AiConfig, model: string) {
    ...
    return Boolean(model.trim() && channel.baseUrl.trim() && (channel.apiFormat === "local" || channel.apiKey.trim()));
}
...
export function applySharedAiConfig(value: Partial<AiConfig>) {
    useConfigStore.setState({ config: normalizeAiConfig(value) });
}
...
export function defaultBaseUrlForApiFormat(apiFormat: ApiCallFormat) {
    ...
    if (apiFormat === "local") return "";
    return OPENAI_BASE_URL;
}

function normalizeApiFormat(apiFormat: unknown): ApiCallFormat {
    return apiFormat === "gemini" || apiFormat === "local" ? apiFormat : "openai";
}
