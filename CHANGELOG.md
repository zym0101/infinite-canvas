...
## Unreleased
+ [新增] 插件市场接入推荐第三方「3D 导演台」节点插件，固定版本并展示 AGPL-3.0 许可证与源码来源，可连接画布图片搭建 3D 场景并回传多视角截图。
+ [优化] 画布多图生成改为单次批量请求，避免浏览器并发重复占用本地 GPU；本地视频节点保存远端任务并支持刷新恢复、排队状态和服务端取消。
+ [新增] 无限画布新增本地 MONOFORM 素形预演节点，支持按节点隔离工程、同步上游参考图，并将 PNG 与 MP4 输出直接持久化为相连的画布媒体节点。
+ [新增] 画布插件 SDK 增加宿主图片读取与图片/视频媒体落盘能力，供本地预演及其他媒体插件复用。
